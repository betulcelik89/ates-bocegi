...
const TreeStages = ({ points }: { points: number }) => {
  if (points < 20) {
    return ( // 1. AŞAMA: SAKSI VE TOPRAK
    ...
    return ( // 3. AŞAMA: GENÇ AĞAÇ
      <svg viewBox="0 0 200 200" className="w-64 h-64 transition-all duration-1000">
        ...
        <circle cx="110" cy="60" r="10" fill="#fbbf24" className="animate-bounce" /> {/* Meyve */}
      </svg>
    );
  } else {
    return ( // 4. AŞAMA: DEV HAYAT AĞACI
      <svg viewBox="0 0 200 200" className="w-80 h-80 drop-shadow-2xl">
        ...
        <circle cx="80" cy="50" r="5" fill="#f87171" className="animate-ping" /> {/* Parlayan Meyveler */}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="bg-white p-8 rounded-[60px] shadow-2xl border-b-[15px] border-teal-500 flex justify-between items-center mb-10">
          <div className="flex items-center gap-6">
            <div className="text-7xl bg-teal-50 p-4 rounded-full border-4 border-teal-100">{studentInfo.avatar_url || '🧒'}</div>
            <div>
              <h1 className="text-4xl font-black uppercase tracking-tighter">{studentInfo.name}!</h1>
          ...
          <div className="bg-tegv-navy p-6 rounded-[40px] text-white text-center shadow-2xl border-t-4 border-teal-400">
            <p className="text-4xl font-black">⭐ {studentInfo.points || 0}</p>
            <p className="text-[9px] font-black uppercase opacity-50">Toplam Puan</p>
          </div>
        </header>
        ...
        {isProfileOpen && (
          <div className="bg-white p-10 rounded-[60px] shadow-2xl mb-10 border-4 border-teal-500 animate-in slide-in-from-top duration-500">
             <h2 className="text-2xl font-black uppercase mb-6 text-center">Ayarların</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          {/* --- SOL KOLON: CANLI AĞAÇ VE ROZETLER --- */}
          <div className="lg:col-span-1 space-y-10">
            
            {/* 🌳 CANLI GELİŞİM AĞACI (Yeni!) */}
            <div className="bg-white p-10 rounded-[60px] shadow-2xl border-t-[12px] border-green-500 overflow-hidden relative">
               ...
               <div className="mt-8 space-y-3">
                  ...
                  <div className="w-full h-5 bg-gray-100 rounded-full overflow-hidden border-2 border-white shadow-sm">
                     <div className="h-full bg-green-500 transition-all duration-1000 shadow-lg shadow-green-200" style={{ width: `${studentInfo.points % 100}%` }}></div>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-tegv-navy p-8 rounded-[50px] shadow-2xl text-white">
               ...
               <div className="flex gap-1">
                  <input value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && askChatbot()} className="flex-1 bg-white/10 p-3 rounded-xl text-xs outline-none focus:border-teal-400" placeholder="Bir şey sor..." />
                  <button onClick={askChatbot} className="bg-teal-500 p-3 rounded-xl hover:scale-110 transition-all shadow-lg">🚀</button>
               </div>
            </div>
          </div>

          {/* --- SAĞ KOLON: ÖDEVLER VE RAPORLAR --- */}
          <div className="lg:col-span-2 space-y-10">
            
            {/* AI RAPORU (ÜSTTE DAHA ŞIK) */}
            ...
            <div className="space-y-6">
              {assignments.map((a) => (
                <div key={a.id} className="bg-white p-10 rounded-[60px] shadow-xl border-l-[20px] border-teal-500 hover:border-tegv-orange transition-all group">
                  <p className="text-[10px] font-black text-teal-600 uppercase mb-4 tracking-widest">{new Date(a.created_at).toLocaleDateString()}</p>
                  <p className="text-2xl font-bold italic mb-8 leading-relaxed group-hover:text-teal-700 transition-colors">&quot;{a.content}&quot;</p>
                  {a.file_url && <a href={a.file_url} target="_blank" className="p-4 bg-teal-50 text-teal-700 rounded-[30px] inline-flex items-center gap-4 hover:bg-tegv-navy hover:text-white transition-all font-black uppercase text-xs">📂 Dosyayı Aç</a>}
                </div>
              ))}
            </div>

            {/* YOKLAMA GRAFİĞİ (En Altta Bir Karnesi Gibi) */}
